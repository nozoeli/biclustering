...
meanRow <- as.vector(apply(errNormalBalaRow, 2, mean))
meanCol <- as.vector(apply(errNormalBalaCol, 2, mean))
lowRow <- as.vector(apply(errNormalBalaRow, 2, quantile, probs = 0.05))
highRow <- as.vector(apply(errNormalBalaRow, 2, quantile, probs = 0.95))
lowCol <- as.vector(apply(errNormalBalaCol, 2, quantile, probs = 0.05))
highCol <- as.vector(apply(errNormalBalaCol, 2, quantile, probs = 0.95))

meanRowRank <- as.vector(apply(errNormalBalaRankRow, 2, mean))
meanColRank <- as.vector(apply(errNormalBalaRankCol, 2, mean))
lowRowRank <- as.vector(apply(errNormalBalaRankRow, 2, quantile, probs = 0.1))
highRowRank <- as.vector(apply(errNormalBalaRankRow, 2, quantile, probs = 0.9))
lowColRank <- as.vector(apply(errNormalBalaRankCol, 2, quantile, probs = 0.1))
highColRank <- as.vector(apply(errNormalBalaRankCol, 2, quantile, probs = 0.9))

plot.new()
plot(mu1, meanRow, pch = 4, col = 2, xlim = c(mu1[1] - 0.05, mu1[length(mu)] + 0.05), ylim = c(0,1), xlab="Elevated Mean",
     ylab="Error Rate", xaxt = 'n')
title('M = 5000, N = 5000, m = 200, n = 400, Normal')
axis(side = 1, at = mu1)
...
plot(mu1, meanRowRank, pch = 4, col = 2, xlim = c(mu1[1] - 0.05, mu1[length(mu)] + 0.05), ylim = c(0,1), xlab="Elevated Mean",
     ylab="Error Rate", xaxt = 'n')
title('M = 500, N = 500, m = 40, n = 30, Normal, Ranked')
axis(side = 1, at = mu1)
