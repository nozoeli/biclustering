...
meanRow <- as.vector(apply(errPoiBalaRow, 2, mean))
meanCol <- as.vector(apply(errPoiBalaCol, 2, mean))
lowRow <- as.vector(apply(errPoiBalaRow, 2, quantile, probs = 0.05))
highRow <- as.vector(apply(errPoiBalaRow, 2, quantile, probs = 0.95))
lowCol <- as.vector(apply(errPoiBalaCol, 2, quantile, probs = 0.05))
highCol <- as.vector(apply(errPoiBalaCol, 2, quantile, probs = 0.95))

meanRowRank <- as.vector(apply(errPoiImbaRow, 2, mean))
meanColRank <- as.vector(apply(errPoiImbaCol, 2, mean))
lowRowRank <- as.vector(apply(errPoiImbaRow, 2, quantile, probs = 0.05))
highRowRank <- as.vector(apply(errPoiImbaRow, 2, quantile, probs = 0.95))
lowColRank <- as.vector(apply(errPoiImbaCol, 2, quantile, probs = 0.05))
highColRank <- as.vector(apply(errPoiImbaCol, 2, quantile, probs = 0.95))

plot.new()
plot(mu1, meanRow, pch = 4, col = 2, xlim = c(mu1[1] - 0.05, mu1[length(mu)] + 0.05), ylim = c(0,1), xlab="Elevated Mean",
     ylab="Error Rate", xaxt = 'n')
title('M = 5000, N = 5000, m = 200, n = 400, Poisson')
axis(side = 1, at = mu1)
...
plot(mu1, meanRowRank, pch = 4, col = 2, xlim = c(mu1[1] - 0.05, mu1[length(mu)] + 0.05), ylim = c(0,1), xlab="Elevated Mean",
     ylab="Error Rate", xaxt = 'n')
title('M = 100, N = 10000, m = 50, n = 300, Poisson')
axis(side = 1, at = mu1)
